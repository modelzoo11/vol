...
class AdminNewsService extends BaseProjectAdminService {
	...
	async getAdminNewsList(admin, {
		...
		let fields = 'NEWS_UNIT_NAME,NEWS_TITLE,NEWS_DESC,NEWS_CATE_ID,NEWS_CATE_NAME,NEWS_EDIT_TIME,NEWS_ADD_TIME,NEWS_ORDER,NEWS_STATUS,NEWS_CATE2_NAME,NEWS_VOUCH,NEWS_QR,NEWS_OBJ';
		...
		if (admin.ADMIN_TYPE == 9) where.and.NEWS_UNIT_ID = admin.ADMIN_UNIT_ID;
		...
		if (util.isDefined(search) && search) {
			...
			switch (sortType) {
				...
				case 'unitId': {
					where.and.NEWS_UNIT_ID = String(sortVal);
					break;
				}
				case 'status': {
