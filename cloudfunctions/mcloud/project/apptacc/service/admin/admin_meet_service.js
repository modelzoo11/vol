...
class AdminMeetService extends BaseProjectAdminService {
	...
	/** 管理员扫码核销 */
	async scanJoin(admin, meetId, code) {
		this.AppError('[医院陪诊]该功能暂不开放，如有需要请加作者微信：cclinux0730');
	}
	...
	async getJoinList({
		...
		let fields = 'JOIN_UNIT_NAME,JOIN_IS_CHECKIN,JOIN_CHECKIN_TIME,JOIN_CODE,JOIN_ID,JOIN_REASON,JOIN_USER_ID,JOIN_MEET_ID,JOIN_MEET_TITLE,JOIN_MEET_DAY,JOIN_MEET_TIME_START,JOIN_MEET_TIME_END,JOIN_MEET_TIME_MARK,JOIN_FORMS,JOIN_STATUS,JOIN_ADD_TIME';
	...
	async getAdminMeetList(admin, {
		...
		let fields = 'MEET_UNIT_NAME,MEET_CATE_ID,MEET_CATE_NAME,MEET_TITLE,MEET_STATUS,MEET_DAYS,MEET_ADD_TIME,MEET_EDIT_TIME,MEET_ORDER,MEET_VOUCH,MEET_QR';
		...
		if (admin.ADMIN_TYPE == 9) where.and.MEET_UNIT_ID = admin.ADMIN_UNIT_ID;

		if (util.isDefined(search) && search) {
			where.or = [
				{ MEET_TITLE: ['like', search] },
				{ MEET_UNIT_NAME: ['like', search] },
			];
		} else if (sortType && util.isDefined(sortVal)) {
			// 搜索菜单
			switch (sortType) {
				case 'status': {
					// 按类型
					where.and.MEET_STATUS = Number(sortVal);
					break;
				}
				case 'unitId': {
					// 按类型
					where.and.MEET_UNIT_ID = sortVal;
					break;
				}
				case 'cateId': {
					// 按类型
					where.and.MEET_CATE_ID = sortVal;
					break;
				}
				case 'vouch': {
					where.and.MEET_VOUCH = 1;
					break;
				}
				case 'top': {
					where.and.MEET_ORDER = 0;
					break;
				}
			}
		}
	...
	/**模板列表 */
	async getMeetTempList(admin, meetId = 'admin') {
		let orderBy = {
			'TEMP_ADD_TIME': 'desc'
		};
		...
		if (admin.ADMIN_TYPE == 9) where.TEMP_UNIT_ID = admin.ADMIN_UNIT_ID;
		else where.TEMP_UNIT_ID = '';
	...
	/**导出报名数据 */
	async exportJoinDataExcel(admin, {
		meetId,
		startDay,
		endDay,
		status
