...
Page({
	...
	_getSearchMenu: async function () {
		...
		if (this.data.admin.type != 9) {
			let unitIdOptions = await UnitBiz.getAllUnitOptions();
			let sortItem2 = [{ label: '医院', type: '', value: '' }];
			for (let k = 0; k < unitIdOptions.length; k++) {
				sortItem2.push({ label: unitIdOptions[k].label, type: 'unitId', value: unitIdOptions[k].val })
			}
